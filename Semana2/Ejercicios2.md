...
#### E2 - Carga correcta de par modelo/tokenizador
...
#### E3 - Decodificación: determinista vs creativa
...
#### E4 - Control de longitud y costo
**Objetivo:** observar efecto de `max_new_tokens`.  
**Pasos:** genera con 20 y 120; mide tiempo "aprox." con `time.time()`.  
**Entrega:** longitudes y tiempos, reflexión (1-2 líneas).


#### E5 - Bucle de chat robusto
...
#### E6 - *Prompting* de tareas con FLAN-T5
...
#### E8 - Dataset mínimo y DataLoader
...
#### E10 - `collate_fn` con `pad_sequence` (batch_first=True)
...
#### E13 - spaCy tokenizers por idioma
...
#### E15 - `collate_fn` (pares) + DataLoader train/valid
...
#### E16 - *Bucketing Sampler* por longitud
...
#### E17 - Métrica de *padding waste* + logging por lote
...
#### E18 - Máscaras de atención y máscara causal del *decoder*
...
### E19 - *Loss* ignorando PAD + *teacher forcing* (bucle mínimo)
...
#### E20 - Sustituir TorchText por *tokenizador HF* (compatibilidad de checkpoint)
...
#### E21 - Portar a `datasets` (HF) + `map` + `DataCollatorWithPadding`
...
#### E22 - *Tests* mínimos (pytest) para el *pipeline* de datos
...
#### E23 - Script CLI de muestreo de lotes
...
#### E24 - *Feature flag* de tokenización (spaCy vs básica vs HF)
...
#### E25 - Truncado seguro + histograma de longitudes
**Objetivo:** evitar OOM y entender distribución.  
**Tareas:** define `max_len` y trunca; genera histograma (binning) de longitudes *antes/después*.  
**Entrega:** histograma textual y reflexión (2-3 líneas).


#### E26 - `collate_fn` "rico" (diccionarios tipados)
...
#### E28 - Métrica de cobertura de vocabulario (OOV rate)
