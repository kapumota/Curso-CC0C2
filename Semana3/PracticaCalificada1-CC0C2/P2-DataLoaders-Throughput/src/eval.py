...
import joblib
def main():
    ap = argparse.ArgumentParser(description="Evalúa el modelo y genera métricas/figuras.")
    ...
    if os.path.exists(model_path):
        ...
        y_true = df[label_col] if label_col is not None else [0]*len(df)
        ...
        if label_col is not None:
            ...
            plt.close()
        print("[OK] Métricas en out/metrics.json")
    else:
        with open(os.path.join(args.out, "metrics.json"), "w") as f:
            json.dump({"nota": "no hay modelo; agrega métricas específicas"}, f, indent=2)
        print("[AVISO] No se encontró modelo; se escribieron métricas placeholder.")
